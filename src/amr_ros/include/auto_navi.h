...
private:
    ...
    ros::ServiceClient                                      srv_client_set_final_angle;
    icartmini_sbtp::target_move_angle                       srv_set_final_angle;
...
public:
    ...
    int SetDestination(const char* nameDestination, double stopAngle);
...
int AutoSetDest::Initialize(node_config *nd_config)
{
    ros::NodeHandle nh;
    ...
    srv_client_set_final_angle       = nh.serviceClient<icartmini_sbtp::target_move_angle>("trajectory_tracking/set_move_angle");
...
int AutoSetDest::SetDestination(const char* nameDestination, double stopAngle)
{
    char start_str[128];
    int start = -1;
    int destination = -1;
    ...
    for(int i = 0 ; i < defAttemptLimits; i++)
    {
        ...
        if( srv_client_find_waypoint.call( srv_find_waypoint ) )
        {
            ...
            //send to trajectory_tracking module to save final stopped angle
            srv_set_final_angle.request.moveTheta = stopAngle;
            if(!srv_client_set_final_angle.call(srv_set_final_angle))
            {
                ROS_ERROR("Request target %s to trajectory tracking failed!",srv_find_waypoint.request.waypoint_name_destination.c_str());
            }
            //send to plan module to run
            srv_set_navigation_path.request.path = path;
            srv_client_set_navigation_path.call(srv_set_navigation_path);
            ROS_INFO("Amr is nearby waypoint: %s",srv_find_waypoint.response.waypoint_name.c_str());
...
int AutoSetDest::GetWaypointPosition(const char *name, double *x, double *y)
{
    return path_net.get_waypoint(name, x, y);
}

bool AutoSetDest::IsCorrectWaypoint(const char *name)
{
    double x, y;
    return (path_net.get_waypoint(name, &x, &y) >= 0) ? true : false;
}

int AutoSetDest::DeleteDestination()
{
    // Call Blank Path.
 ...
 */
bool AutoSetDest::path_plan( gnd::path::path_net_area_and_speed_limited *path_net, int start, int destination, gnd_msgs::msg_path_area_and_speed_limited *path )
{
